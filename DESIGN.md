...
- **Flagship vehicle** (profile panel section): real photography where a verifiably free license exists (`src/data/vehicle-photos.ts`, sources in ATTRIBUTIONS.md) shown in a cover-cropped inset box with a visible credit line ("Photo: SpaceX · CC0"); Expand opens the uncropped photograph in a glass-stage overlay (Esc/backdrop closes). Beneath it, a real-figures spec grid (height, diameter, LEO payload, stages, engines, propellant, first flight — tabular numerals, conservative values marked ≈) for every vehicle. Photo-or-nothing rule: vehicles without a cleanly licensed photo get the spec grid only — no placeholder art, no license grey areas.
