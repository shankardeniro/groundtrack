...
- **Vehicle viewer** (inline in the profile panel + expandable overlay): stylized procedural 3D launchers (three.js) built from declarative specs in `src/data/vehicles.ts` — recognizable silhouettes carry per-vehicle features (grid fins, stowed landing legs, aft strakes, boosters, livery bands, an accent glow ring at the engines) under image-based lighting with a livery-accent rim. Slow idle spin (off under reduced motion), drag to rotate, Expand opens a large glass-stage overlay (Esc/backdrop closes). A real-figures spec grid (height, diameter, LEO payload, stages, engines, propellant, first flight — tabular numerals, conservative values marked ≈) sits beneath the viewer. The caption always says "stylized depiction" — the geometry is design language; the numbers are real.
